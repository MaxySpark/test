...
extends com.arlabs.rapidplay.definitions.Repository[User] {

  override def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection[JSONCollection](s"user"))
    

  private val logger = Logger(this.getClass)

  override def getOne(filter: JsObject)(implicit mc: MarkerContext): Future[Option[User]] = {
    collection.flatMap (_.find(filter).one[User])
  }

  override def get(filter: JsObject, order: Int, skipN: Int, limitN: Int)(implicit mc: MarkerContext): Future[List[User]] = {
    ...
    collection.flatMap (_.find(filter)
      ...
      .batchSize(limitN)
      .cursor[User]()
      .collect[List](limitN,Cursor.FailOnError[List[User]]()))
  }
  override def get(filter: JsObject, order: Int, skipN: Int)(implicit mc: MarkerContext): Future[List[User]] = {
  ...
  override def create(data: User)(implicit mc: MarkerContext): Future[Boolean] = {
    collection.flatMap(_.insert(data)).map(_.ok)
  }

  override def upsert(filter: JsObject, data: User)(implicit mc: MarkerContext): Future[Boolean] = {
