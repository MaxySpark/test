...
def create()(implicit request: AppRequest): Future[Result] = {
  request.body.asJson match {
    case Some(jsValue) =>
      jsValue.validate[DemomodelCreate] match {
        case s: JsSuccess[DemomodelCreate] =>
          model.create(s.get)
        case e: JsError =>
...
class Router @Inject()(controller: Controller, appAction: AppAction, model: Model)(implicit val executionContext: ExecutionContext) extends SimpleRouter {
  override def routes: Routes = {
    case GET(p"/aggr") =>
      ...
      appAction.run {
        implicit request =>
        model.getDirect("{where:{}}").map(demomodel => {

        val tempFileName = "Demomodel.xls"
        val tempFile =  TempFile.create("Demomodel", ".xls")
        
        ExportUtil.exportToExcel(Demomodel.keys, demomodel ,tempFile)
