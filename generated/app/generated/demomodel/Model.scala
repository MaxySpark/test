...
class Model @Inject()(repositoryImpl: RepositoryImpl)
 extends ModelDefinition[Demomodel, com.maxyspark.something.generated.user.User, hooks.AppRequest] {
  /* validators start */
  ()
  ...
  override def createHooks(implicit r: hooks.AppRequest): List[Demomodel => Unit] = List()
  ...
  def createValidators(implicit userSession: UserSession[com.maxyspark.something.generated.user.User], appRequest: hooks.AppRequest): List[Demomodel => Future[Boolean]] = List()
  def readValidators(implicit userSession: UserSession[com.maxyspark.something.generated.user.User]): List[Demomodel => Future[Boolean]] = List()

  override def repository: RepositoryImpl = repositoryImpl
  override def validate(model: Demomodel)(implicit session: com.arlabs.rapidplay.abstraction.UserSession[com.maxyspark.something.generated.user.User]): Future[Boolean] = Future{
    true
  }
  ...
  def create(demomodelCreate: DemomodelCreate)(implicit request: hooks.AppRequest): Future[play.api.mvc.Result] = {
    request.user match {
      case Some(session) =>
        implicit val implicitSession: UserSession[com.maxyspark.something.generated.user.User] = session
        val demomodel = demomodelCreate.toDemomodel
        executeCreateHooks(demomodel)
        createValidate(demomodel).flatMap(if(_) repository.create(demomodel).map(if (_) demomodel.toPublicOk else play.api.mvc.Results.ServiceUnavailable) else Future{play.api.mvc.Results.Forbidden("It seems you are not allowed to perform this action.")})
      case None =>
        Future{
  ...
  def createDirect(demomodelCreate: DemomodelCreate)(implicit request: hooks.AppRequest): Future[Option[DemomodelPublic]] = {
    request.user match {
      case Some(session) =>
        implicit val implicitSession: UserSession[com.maxyspark.something.generated.user.User] = session
        val demomodel = demomodelCreate.toDemomodel
        executeCreateHooks(demomodel)
        createValidate(demomodel).flatMap(if(_) repository.create(demomodel).map(if (_) Some(demomodel.toPublic) else None) else Future{None})
      case None =>
        Future{
          None
        }
  ...
  def get(id: java.util.UUID)(implicit request: hooks.AppRequest): Future[play.api.mvc.Result] = {
    request.user match {
      case Some(session) =>
        ...
        repository.getOne(query).map(a => if (a.isDefined) a.get.toPublicOk else ApplicationResult.NotFound(s"(Demomodel:$id)"))
      case None =>
        Future{
          ApplicationResult.Forbidden()
        }
  ...
  def getDirect(filterQ: String, order: Int = 1, skipN: Int = 0, limitN: Int = 10)(implicit request: hooks.AppRequest): Future[List[DemomodelPublic]] = {
    implicit val operationType = "READ"
    val filter = genQuery(filterQ)
  ...
  def update(id: UUID, updateQ: JsObject)(implicit request: hooks.AppRequest): Future[play.api.mvc.Result] = {
    checkQuery(updateQ)
    require(updateQ.keys.forall(Demomodel.keys.contains), " Tried keys are: " + updateQ.keys.mkString(", ") +  "Difference is: " + updateQ.keys.diff(Demomodel.keys).mkString(", "))
    request.user match {
      case Some(session) =>
        implicit val implicitSession: UserSession[com.maxyspark.something.generated.user.User] = session
        val query: JsObject = {
